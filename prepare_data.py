...
class Cityscapes():  
    ...
    def __init__(self, **kwargs):
        ...
        self.images = []    # where to load input images 
        self.targets = []   # where to load ground truth 
        self.filename = []  # image filename
        self.preds = []     # where to load semantic segmentation predictions on clean data
        self.preds_adv = [] # where to load semantic segmentation predictions on perturbed data
        ...
        for city in sorted(os.listdir(os.path.join(CONFIG.DATA_DIR, 'Cityscapes', 'leftImg8bit', 'val'))):
            for img in sorted(os.listdir(os.path.join(CONFIG.DATA_DIR, 'Cityscapes', 'leftImg8bit', 'val', city))):
                ...
                self.preds.append(os.path.join(CONFIG.PROBS_DIR, img.replace('png','npy'))) 
                self.preds_adv.append(os.path.join(CONFIG.PROBSA_DIR, img.replace('png','npy'))) 
